...
// Using global GSAP from CDN for maximum reliability in this environment
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (gsap) {
        gsap.to(spotlight, {
          left: e.clientX,
          top: e.clientY,
          opacity: minDistance <= proximity ? 0.8 : minDistance <= fadeDistance ? ((fadeDistance - minDistance) / (fadeDistance - proximity)) * 0.8 : 0,
          duration: 0.2
        });
      }
    };
      ...
      {data.map((card, index) => {
        ...
        return (
          <ParticleCard
            ...
            onClick={() => navigate(`/modules/${card.id}`)}
          >
